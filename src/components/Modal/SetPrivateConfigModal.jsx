...
const SetPrivateConfigModal = ({
  ...
  useEffect(() => {
    axios
      .get(
        `${baseUrl}api/sub-client-area/set-config/set-tg-private-alert-code/${openSetConfigModal.value.id}`,
        {
          headers: {
    ...
    const interval = setInterval(async () => {
      try {
        const response = await axios.get(
          `${baseUrl}api/sub-client-area/set-config/get-tg-private-status/${openSetConfigModal.value.id}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            <p className="text-green-500 inline">
              {configDetails?.data?.token}
            </p>
            <FaCopy
              onClick={() => {
                navigator.clipboard.writeText(configDetails?.data?.token);
...
export default SetPrivateConfigModal;
